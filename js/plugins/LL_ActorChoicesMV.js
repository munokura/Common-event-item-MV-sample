 ...
 *
 * プラグインコマンド:
 *   LL_ActorChoicesMV [選択種] [変数] [キャンセル] [背景] [位置] [リストID]
 *
 *   選択種: partyMember=現在のパーティ、
 *           deadPartyMember=戦闘不能のパーティ、
 *           alivePartyMember=生存しているパーティ、
 *           secondPartyMember=先頭を除くパーティ、
 *           reserveMember=パーティ未参加メンバー、
 *           allMember=全てのメンバー
 *
 *   変数: 選択結果を受け取る変数IDを指定します。
 *   キャンセル: 0=禁止、1=許可 (キャンセルされた場合、結果は-1)
 *   背景: 0=ウィンドウ、1=暗くする、2=透明
 *   位置: 0=左、1=中、2=右
 *   リストID(省略可): 絞込リストのIDを指定
 *
 *   【プラグインコマンドの入力例】
